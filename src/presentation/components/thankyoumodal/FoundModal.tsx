...
const FoundModal= ({open, onClose, children}:FoundModal) => {
  if (!open) {
    return null;
  }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black bg-opacity-50 font-serif">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-sm p-8 text-center">
        ...
        <h2 className="text-2xl font-bold mb-4">Item Posted Successfully!</h2>
        <p className="text-gray-700 mb-8">
          Your report has been saved. The item is now marked “Found – Unclaimed” and included in the matching checklist.
        </p>
        <button
          onClick={onClose}
