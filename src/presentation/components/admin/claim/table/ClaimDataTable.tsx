...
const ClaimDataTable: React.FC<ClaimDataTableProps> = ({ items }) => {
  ...
  const getStatusDisplay = (status?: string) => {
    ...
    return (
      <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${bgClass} text-white`}>
        {label}
      </span>
    );
  };
  ...
  return (
    <div className="bg-[#0f172a] rounded-xl shadow-lg p-4 sm:p-6 w-full overflow-x-auto">
      ...
      {/* Make entire table layout responsive without changing design */}
            ...
            return (
              ...
              >
                ...
                <div className="flex items-center gap-3">
                  <img className="h-10 w-10 rounded-full object-cover md:h-12 md:w-12" src={imageUrl} alt={displayItem.category} />
                  <div className="space-y-0.5 md:space-y-1">
                    <div className="font-medium">{displayItem.firstName} {displayItem.lastName}</div>
                    <div className="text-gray-400 text-[9px] sm:text-xs">{displayItem.email}</div>
                    <div className="text-gray-400 text-[9px] sm:text-xs">{displayItem.contactNumber}</div>
                  </div>
                </div>
                ...
                <div className="flex justify-center">{getStatusDisplay(item.status)}</div>
              </div>
            );
          })}
        </div>
